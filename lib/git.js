...
export function getCommits(repoPath, author, startDate, endDate) {
  try {
    ...
    // 使用 --all 读取所有分支的提交记录
    const cmd = `git -C "${repoPath}" log --all ${authorFilter} --since="${startDate}" --until="${endDate} 23:59:59" --pretty=format:"%H|%ad|%s" --date=short`;
...
export function getCommitDiff(repoPath, commitHash) {
  try {
    ...
    // 获取更完整的 diff（限制大小避免 Prompt 过长）
    const diffCmd = `git -C "${repoPath}" show ${commitHash} --format="" -U2`;
    let diff = '';
    try {
      diff = execSync(diffCmd, { encoding: 'utf-8', maxBuffer: 1024 * 1024 });
      // 限制 diff 长度为 1500 字符，避免 Prompt 超出模型限制
      if (diff.length > 1500) {
